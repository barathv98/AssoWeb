...
const CartAddress: FC<CartAddressProps> = () => {
	...
	const { placeOrder } = useRequestPlaceOrder({
		...
		params: {
			...
			city: city.value,
			pincode: pincode.value.trim().replace(/\s/g, ''),
			district: district,
			state: state,
	...
	useEffect(() => {
		if (pincodeRegex.test(pincode.value)) getPODetails();
	}, [pincode.value, getPODetails]);

	useEffect(() => {
		if (name.value && address.value && city.value && pincode.value) setOrderBtnDisabled(false);
		else setOrderBtnDisabled(true);
	...
	if (!isLoading && !orderState && !cart.length)
		return <div className={styles.unAuthenticated}>Your cart is empty. Please add items to place order</div>;
	...
	return (
		<div className={styles.cartAddress}>
				...
				<div className={styles.sameRowFields}>
					...
					<div className={styles.pincodeField}>
						<Input
							...
							errorMessage={formErrors?.pincode ? formErrors?.pincode : ''}
							{...pincode}
						/>
					</div>
				</div>
			...
			<div className={styles.orderButtonRow}>
				<button
					className={`${styles.orderButton} ${orderBtnDisabled ? styles.disabled : styles.enabled}`}
				...
				<Modal
					...
					content={
						orderState === 'loading' ? (
							...
							<ConfirmationContent success={orderState === 'success'} closePopup={onCloseOrderPopup} />
						)
					}
